...
from matplotlib import image, pyplot, transforms
import time
import random
import numpy as np
import math
...
xr=0; yr=0						# robot's initial position
xr_old=xr; yr_old=yr 					# robot's previous coordinates
xr_new=0; yr_new=0					# robot's next coordinates 
...
goal = [(1.5, 3.2), (2.7, 4), (4, 3.9), (5.1, 3.2)]	# robot goal positions
...
for i in range(0, n_goal):
  ...
  while True:
    ...
    phi_desired=np.arctan2((yg-yr_old),(xg-xr_old))	        # arctan2 tiene conto dei segni risultando nel quadrante giusto 
    ...
    for pixelPoint in path.vertices:
      ...
      fig = plt.figimage(rotated,pixelPoint[0]-sprite_size[0]/2,pixelPoint[1]-sprite_size[1]/2,origin="upper")
